(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))o(n);new MutationObserver(n=>{for(const l of n)if(l.type==="childList")for(const s of l.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&o(s)}).observe(document,{childList:!0,subtree:!0});function t(n){const l={};return n.integrity&&(l.integrity=n.integrity),n.referrerPolicy&&(l.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?l.credentials="include":n.crossOrigin==="anonymous"?l.credentials="omit":l.credentials="same-origin",l}function o(n){if(n.ep)return;n.ep=!0;const l=t(n);fetch(n.href,l)}})();const D=.09,L=1e3;function b(i){return+(Math.round(+(i+"e2"))+"e-2")}function J(i){const e=Math.round(+(i+"e2"));return+(Math.round(e/5)*5+"e-2")}function k(i){return b(i*D)}function T(i){return b(i.quantity*i.unitPriceExGst)}function P(i,e={}){const t=e.method??"per_line",o=b(Math.max(e.discountExGst??0,0)),n=b(i.reduce((f,g)=>f+T(g),0)),l=b(Math.max(n-o,0)),s=i.map(f=>k(T(f)));let m;if(t==="per_line")if(n>0&&o>0){const f=l/n;m=b(s.reduce((g,$)=>g+$*f,0))}else m=b(s.reduce((f,g)=>f+g,0));else m=k(l);let u=b(l+m);return e.roundCash&&(u=J(u)),{subtotalExGst:n,discountExGst:o,taxableExGst:l,gstAmount:m,totalInclGst:u,lineGst:s}}function O(i,e,t){return e.isCreditNote?"credit_note":i.gstRegistered?e.customer.gstRegistered?t.totalInclGst<=L?"simplified_tax_invoice":"tax_invoice":"receipt":"invoice"}function V(i){return{invoice:"INVOICE",tax_invoice:"TAX INVOICE",simplified_tax_invoice:"TAX INVOICE",receipt:"RECEIPT",credit_note:"CREDIT NOTE"}[i]}function X(i){return i==="simplified_tax_invoice"||i==="receipt"}function Y(i,e,t,o){const n=[];return i.name.trim()||n.push({field:"profile.name",message:"Business name is required."}),i.address.trim()||n.push({field:"profile.address",message:"Business address is required."}),e.invoiceNumber.trim()||n.push({field:"invoice_number",message:"Invoice number is required."}),e.date.trim()||n.push({field:"date",message:"Invoice date is required."}),e.lineItems.length||n.push({field:"line_items",message:"At least one line item is required."}),t==="invoice"?n:t==="credit_note"?(e.originalInvoiceNumber.trim()||n.push({field:"original_invoice_number",message:"Original invoice number is required on a credit note."}),i.gstRegistered&&!i.gstRegistrationNumber.trim()&&n.push({field:"profile.gst_registration_number",message:"GST registration number is required for GST documents."}),n):i.gstRegistered?(i.gstRegistrationNumber.trim()||n.push({field:"profile.gst_registration_number",message:"GST registration number is required for GST documents."}),(t==="tax_invoice"||t==="simplified_tax_invoice")&&(e.customer.name.trim()||n.push({field:"customer.name",message:"Customer name is required on a tax invoice."}),t==="tax_invoice"&&!e.customer.address.trim()&&n.push({field:"customer.address",message:"Customer address is required on a full tax invoice."})),t==="receipt"&&o.totalInclGst<=0&&n.push({field:"total",message:"Receipt total must be greater than zero."}),n):(n.push({field:"profile.gst_registered",message:"GST-registered mode requires GST registration to be enabled."}),n)}function v(i,e="SGD"){return new Intl.NumberFormat("en-SG",{style:"currency",currency:e,minimumFractionDigits:2}).format(i)}function A(i){return i?new Date(i+"T12:00:00").toLocaleDateString("en-SG",{day:"2-digit",month:"short",year:"numeric"}):""}function p(i){return i.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const F="sg-invoice-app-v1",w=()=>({name:"",address:"",phone:"",email:"",uen:"",gstRegistrationNumber:"",gstRegistered:!1,bankName:"",bankAccount:"",paynow:"",invoicePrefix:"INV",calculationMethod:"per_line",roundCashToFiveCents:!1}),h=()=>({invoiceNumber:"",date:new Date().toISOString().slice(0,10),dueDate:"",paymentTerms:"Payment due within 14 days",notes:"",currency:"SGD",lineItems:[{id:crypto.randomUUID(),description:"",quantity:1,unitPriceExGst:0}],customer:{name:"",address:"",gstRegistered:!1},discountExGst:0,isCreditNote:!1,originalInvoiceNumber:""});function G(){return{profile:w(),invoice:h(),nextSequence:1}}function K(){var i,e,t;try{const o=localStorage.getItem(F);if(!o)return G();const n=JSON.parse(o);return{...G(),...n,profile:{...w(),...n.profile},invoice:{...h(),...n.invoice,customer:{...h().customer,...(i=n.invoice)==null?void 0:i.customer},lineItems:(t=(e=n.invoice)==null?void 0:e.lineItems)!=null&&t.length?n.invoice.lineItems:h().lineItems}}}catch{return G()}}function M(i){localStorage.setItem(F,JSON.stringify(i))}function Q(i){return JSON.stringify({exportedAt:new Date().toISOString(),profile:i.profile,invoice:i.invoice,nextSequence:i.nextSequence},null,2)}function W(i){var t;const e=JSON.parse(i);return{profile:{...w(),...e.profile},invoice:{...h(),...e.invoice,customer:{...h().customer,...(t=e.invoice)==null?void 0:t.customer}},nextSequence:e.nextSequence??1}}function B(i,e,t){return`${i}-${e}-${String(t).padStart(4,"0")}`}function U(i){const{profile:e,invoice:t}=i,o=P(t.lineItems,{discountExGst:t.discountExGst,method:e.calculationMethod,roundCash:e.roundCashToFiveCents}),n=O(e,t,o),l=V(n),s=e.gstRegistered,m=n==="simplified_tax_invoice",u=n==="receipt",f=n==="credit_note",g=t.lineItems.map(y=>{const E=T(y),q=s?o.lineGst[t.lineItems.indexOf(y)]??0:0,C=s?E+q:E;return m||u?`
        <tr>
          <td>${p(y.description||"—")}</td>
          <td class="num">${y.quantity}</td>
          <td class="num">${v(C,t.currency)}</td>
        </tr>`:`
        <tr>
          <td>${p(y.description||"—")}</td>
          <td class="num">${y.quantity}</td>
          <td class="num">${v(y.unitPriceExGst,t.currency)}</td>
          ${s?`<td class="num">${v(q,t.currency)}</td>`:""}
          <td class="num">${v(s?C:E,t.currency)}</td>
    ...
    <article class="invoice-doc" data-doc-type="${n}">
      <header class="invoice-doc__header">
        ...
        <div class="invoice-doc__meta">
          ...
          <p><strong>Date</strong> ${A(t.date)}</p>
          ${t.dueDate?`<p><strong>Due</strong> ${A(t.dueDate)}</p>`:""}
      ...
      <section class="invoice-doc__totals">
        ...
        <div class="totals-row"><span>GST @ ${D*100}%</span><span>${v(o.gstAmount,t.currency)}</span></div>`:""}
        `:""}
        <div class="totals-row totals-row--grand">
          <span>Total payable</span>
          <span>${v(s?o.totalInclGst:o.taxableExGst,t.currency)}</span>
        </div>
        ${X(n)?'<p class="gst-statement">Price payable includes GST</p>':""}
      </section>
  ...
  `}function R(i){return{invoice:"Standard invoice",tax_invoice:"Full tax invoice",simplified_tax_invoice:`Simplified tax invoice (≤ $${L.toLocaleString()})`,receipt:"Receipt (non-GST customer)",credit_note:"Credit note"}[i]}let a=K(),S="profile";function N(){return P(a.invoice.lineItems,{discountExGst:a.invoice.discountExGst,method:a.profile.calculationMethod,roundCash:a.profile.roundCashToFiveCents})}function I(){return O(a.profile,a.invoice,N())}function j(){return Y(a.profile,a.invoice,I(),N())}function Z(){const i=N(),e=I(),t=j(),o=R(e),n=document.getElementById("preview-root");n&&(n.innerHTML=U(a));const l=document.getElementById("preview-badge");l&&(l.textContent=o,l.className=`badge${t.length?" badge--warn":""}`);const s=document.getElementById("form-doc-type-badge");s&&(s.textContent=o);const m=document.getElementById("inv-summary");m&&(m.textContent=a.profile.gstRegistered?`GST: ${v(i.gstAmount)} · Total: ${v(i.totalInclGst)}`:`Total: ${v(i.taxableExGst)}`);const u=document.getElementById("validation-container");u&&(u.innerHTML=t.length?`<ul class="validation-list">${t.map(f=>`<li>${d(f.message)}</li>`).join("")}</ul>`:"")}function x(){M(a),Z()}function _(){M(a),z()}function H(){const i=new Date().getFullYear();a.invoice.invoiceNumber||(a.invoice.invoiceNumber=B(a.profile.invoicePrefix,i,a.nextSequence),a.nextSequence+=1)}function c(i,e,t,o=!1){i.addEventListener("input",()=>{const n=i.type==="checkbox"?i.checked:i.value;i.type==="number"?t(parseFloat(i.value)||0):typeof e()=="boolean"?t(n):typeof e()=="number"?t(parseFloat(i.value)||0):t(n),o?_():x()})}function ee(i){const e=a.profile;i.innerHTML=`
  ...
  `,c(r("#biz-name"),()=>e.name,t=>{e.name=t}),c(r("#biz-address"),()=>e.address,t=>{e.address=t}),c(r("#biz-phone"),()=>e.phone,t=>{e.phone=t}),c(r("#biz-email"),()=>e.email,t=>{e.email=t}),c(r("#biz-uen"),()=>e.uen,t=>{e.uen=t}),c(r("#biz-prefix"),()=>e.invoicePrefix,t=>{e.invoicePrefix=t}),c(r("#biz-gst-reg"),()=>e.gstRegistered,t=>{e.gstRegistered=t},!0),c(r("#biz-gst-no"),()=>e.gstRegistrationNumber,t=>{e.gstRegistrationNumber=t}),c(r("#biz-calc"),()=>e.calculationMethod,t=>{e.calculationMethod=t}),c(r("#biz-cash-round"),()=>e.roundCashToFiveCents,t=>{e.roundCashToFiveCents=t}),c(r("#biz-bank"),()=>e.bankName,t=>{e.bankName=t}),c(r("#biz-account"),()=>e.bankAccount,t=>{e.bankAccount=t}),c(r("#biz-paynow"),()=>e.paynow,t=>{e.paynow=t})}function te(i){const e=a.invoice,t=N(),o=I();i.innerHTML=`
    <p id="form-doc-type-badge" class="badge">${d(R(o))}</p>
  ...
  `,c(r("#inv-no"),()=>e.invoiceNumber,s=>{e.invoiceNumber=s}),c(r("#inv-date"),()=>e.date,s=>{e.date=s}),c(r("#inv-due"),()=>e.dueDate,s=>{e.dueDate=s}),c(r("#inv-terms"),()=>e.paymentTerms,s=>{e.paymentTerms=s}),c(r("#cust-name"),()=>e.customer.name,s=>{e.customer.name=s}),c(r("#cust-gst"),()=>e.customer.gstRegistered,s=>{e.customer.gstRegistered=s}),c(r("#cust-addr"),()=>e.customer.address,s=>{e.customer.address=s}),c(r("#inv-discount"),()=>e.discountExGst,s=>{e.discountExGst=s}),c(r("#inv-notes"),()=>e.notes,s=>{e.notes=s}),c(r("#inv-credit-note"),()=>e.isCreditNote,s=>{e.isCreditNote=s},!0);const n=document.querySelector("#inv-orig-no");n&&c(n,()=>e.originalInvoiceNumber,s=>{e.originalInvoiceNumber=s}),r("#btn-new-no").addEventListener("click",()=>{const s=new Date().getFullYear();a.invoice.invoiceNumber=B(a.profile.invoicePrefix,s,a.nextSequence),a.nextSequence+=1,r("#inv-no").value=a.invoice.invoiceNumber,x()});const l=r("#line-items");ie(l),r("#btn-add-line").addEventListener("click",()=>{e.lineItems.push({id:crypto.randomUUID(),description:"",quantity:1,unitPriceExGst:0}),_()})}function ie(i){i.innerHTML=a.invoice.lineItems.map((e,t)=>`
    ...
    </div>`).join(""),i.querySelectorAll(".line-item").forEach(e=>{var n,l,s,m;const t=e.dataset.id,o=a.invoice.lineItems.find(u=>u.id===t);(n=e.querySelector('[data-field="desc"]'))==null||n.addEventListener("input",u=>{o.description=u.target.value,x()}),(l=e.querySelector('[data-field="qty"]'))==null||l.addEventListener("input",u=>{o.quantity=parseFloat(u.target.value)||0,x()}),(s=e.querySelector('[data-field="price"]'))==null||s.addEventListener("input",u=>{o.unitPriceExGst=parseFloat(u.target.value)||0,x()}),(m=e.querySelector("[data-remove]"))==null||m.addEventListener("click",()=>{a.invoice.lineItems.length>1&&(a.invoice.lineItems=a.invoice.lineItems.filter(u=>u.id!==t),_())})})}function d(i){return i.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function r(i){return document.querySelector(i)}function z(){const i=j(),e=r("#app");e.innerHTML=`
    ...
    <div class="layout layout--split">
      <section class="panel">
        ...
        <div id="validation-container" class="panel__body" style="padding-top:0">
          ${i.length?`<ul class="validation-list">${i.map(o=>`<li>${d(o.message)}</li>`).join("")}</ul>`:""}
        </div>
      </section>
      ...
      <section class="panel">
        <div class="panel__header no-print">
          <h2>Preview</h2>
          <span id="preview-badge" class="badge ${i.length?"badge--warn":""}">${d(R(I()))}</span>
        </div>
  ...
  `,e.querySelectorAll("[data-tab]").forEach(o=>{o.addEventListener("click",()=>{S=o.dataset.tab,z()})});const t=r("#form-root");S==="profile"?ee(t):(a.invoice.invoiceNumber||H(),te(t)),r("#btn-print").addEventListener("click",()=>window.print()),r("#btn-export").addEventListener("click",()=>{const o=new Blob([Q(a)],{type:"application/json"}),n=document.createElement("a");n.href=URL.createObjectURL(o),n.download=`invoice-${a.invoice.invoiceNumber||"backup"}.json`,n.click(),URL.revokeObjectURL(n.href)}),r("#btn-import").addEventListener("change",async o=>{var s;const n=(s=o.target.files)==null?void 0:s[0];if(!n)return;const l=await n.text();a=W(l),_()})}H();z();
